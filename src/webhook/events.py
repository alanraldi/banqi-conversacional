...
Cada função recebe o payload do evento e retorna a mensagem a ser enviada ao cliente
via WhatsApp. Retorna None se nenhuma mensagem precisar ser enviada.
"""
...
def handle_consent_term_file_ready(data: dict[str, Any]) -> str | None:
    ...
    Deve enviar o link do PDF ao cliente e pedir aceitação.
    """
...
def handle_no_offer_available(data: dict[str, Any]) -> str | None:
    ...
    O errorCode determina a mensagem ao cliente.
    """
    error_code = data.get("errorCode") or data.get("error_code", "")

    messages = {
        "PDF_GENERATION_ERROR": (
            "Tivemos um problema técnico ao gerar seu documento. "
            "Tente novamente em alguns instantes."
        ),
        "TOKEN_GENERATION_ERROR": (
            "Não conseguimos verificar seus dados no momento. "
            "Tente novamente mais tarde."
        ),
        "ELIGIBILITY_REJECTED": (
            "Infelizmente não encontramos uma oferta de empréstimo consignado "
            "disponível para você agora. Qualquer dúvida, estamos aqui para ajudar."
        ),
        "SIMULATION_ERROR": (
            "Tivemos um problema ao simular o empréstimo. "
            "Tente novamente em alguns instantes."
        ),
    }

    msg = messages.get(error_code)
    if not msg:
        logger.warning("handle_no_offer_available: errorCode desconhecido '%s'", error_code)
...
def handle_simulation_ready(data: dict[str, Any]) -> str | None:
    ...
    Apresenta a oferta e pergunta se o cliente aceita ou quer ajustar.
    """
...
def handle_proposal_status_update(data: dict[str, Any]) -> str | None:
    ...
    disbursement_date = (
        data.get("disbursementDate")
        or data.get("data", {}).get("disbursementDate", "")
    )
    amount = (
        data.get("amount")
    ...
    messages = {
        ...
        "PENDING_DISBURSEMENT": (
            f"Desembolso agendado para {disbursement_date}. 📅" if disbursement_date
            else "Desembolso agendado. Em breve o valor estará na sua conta. 📅"
        ),
        "DISBURSED": (
            f"R$ {amount} creditado na sua conta banQi! Bom proveito! 🎉" if amount
            else "Valor creditado na sua conta banQi! Bom proveito! 🎉"
        ),
        "CANCELED": "Sua proposta foi cancelada. Precisa de ajuda? Estou aqui!",
        "ERROR": (
            "Ocorreu um erro no processamento da sua proposta. "
