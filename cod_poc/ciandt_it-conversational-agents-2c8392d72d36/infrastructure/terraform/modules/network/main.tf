# -----------------------------------------------------------------------------
# Network Module — Dual mode: create new VPC or use existing
# vpc_mode = "create"   → creates VPC, subnets, IGW, NAT, route tables
# vpc_mode = "existing" → looks up VPC by name, uses existing subnets
# Both modes create: Security Groups, VPC Endpoints, WAF
# -----------------------------------------------------------------------------
...
# =============================================================================
# SHARED — Security Groups (both modes)
# =============================================================================
...
resource "aws_vpc_security_group_egress_rule" "lambda_to_runtime" {
  ...
  to_port                      = 8080
  ip_protocol                  = "tcp"
  description                  = "Allow to AgentCore Runtime"
}
...
resource "aws_vpc_security_group_egress_rule" "runtime_to_internet" {
  ...
  to_port           = 443
  ip_protocol       = "tcp"
  description       = "Allow HTTPS to internet via NAT (external APIs)"
}
...
# =============================================================================
# SHARED — VPC Endpoints (both modes)
# =============================================================================
...
resource "aws_vpc_endpoint" "aoss" {
  ...
  tags = { Name = "${local.name_prefix}-vpce-aoss" }
}
