...
# Protocolo: MCP (Model Context Protocol) com busca semântica
# -----------------------------------------------------------------------------
...
output "gateway_url"         { value = aws_bedrockagentcore_gateway.this.endpoint_url }
output "oauth_client_id"     { value = aws_cognito_user_pool_client.gateway.id }
...
variable "banqi_api_base_url" { type = string }
