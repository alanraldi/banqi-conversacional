...
def create_consent_term(name: str, phone: str, cpf: str) -> dict[str, Any]:
    ...
    Returns:
        dict com campos:
        - status: "PENDING" | "ALREADY_ACTIVE" | "TOO_MANY_CPFS" | "ERROR"
    ...
    try:
        ...
        return {"status": "ERROR", "message": "Timeout ao conectar com a API banQi. Tente novamente.", "http_status": None}
    except httpx.RequestError as exc:
        logger.error("create_consent_term: erro de rede — %s", exc)
        return {"status": "ERROR", "message": f"Erro de conexão com a API banQi: {exc}", "http_status": None}
...
def accept_consent_term(phone: str, cpf: str, ip: str, user_agent: str) -> dict[str, Any]:
    ...
    try:
        ...
        if resp.status_code == 200:
            return {
                "status": "ACCEPTED",
                "message": "Termo de consentimento aceito. Aguardando webhook SIMULATION_READY ou NO_OFFER_AVAILABLE.",
                "http_status": 200,
            }
