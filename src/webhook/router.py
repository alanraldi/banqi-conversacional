...
Recebe o payload do backend, identifica o tipo de evento e despacha
para o handler correto. Retorna a mensagem a ser enviada ao cliente
ou None se nenhuma ação for necessária.
"""
...
def route_banqi_webhook(event_type: str, payload: dict[str, Any]) -> str | None:
    ...
    Args:
        event_type: Tipo do evento (ex: "CONSENT_TERM_FILE_READY").
        payload: Payload completo do webhook.

    Returns:
        Mensagem a enviar ao cliente, ou None se não houver mensagem.
    """
    ...
    try:
        ...
        logger.error(
            "route_banqi_webhook: erro no handler do evento '%s' — %s",
            event_type,
            exc,
            exc_info=True,
        )
