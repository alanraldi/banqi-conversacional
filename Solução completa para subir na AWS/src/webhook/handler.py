...
def _handle_whatsapp_message(body_bytes: bytes, headers: dict[str, str]) -> dict[str, Any]:
    ...
    error_message = os.environ.get("ERROR_MESSAGE_GENERIC", "Desculpe, ocorreu um erro. Tente novamente em alguns instantes.")
    ...
    for msg in messages:
        ...
        try:
            response_text = invoke_agent_runtime(prompt=msg.text.body, user_id=phone, session_id=session_id)
        except RuntimeError:
            response_text = error_message

        if memory_id and response_text != error_message:
            save_conversation_to_memory(memory_id=memory_id, actor_id=phone, session_id=session_id, user_message=msg.text.body, agent_response=response_text)
...
def _handle_banqi_webhook(body_bytes: bytes, headers: dict[str, str], method: str) -> dict[str, Any]:
